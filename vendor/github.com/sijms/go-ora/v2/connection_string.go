...
// url.Parse isn't suitable for character '#' in password, therefore use newUrlParse to replace url.Parse code
func newUrlParse(rawURL string) (*url.URL, error) {
	var path string
	var host string
	strs := strings.Split(rawURL, "://")
	schema := strs[0]
	...
	return &url.URL{Scheme: schema, User: user, Host: host, Path: path, ForceQuery: false}, nil
}
...
func newConnectionStringFromUrl(databaseUrl string) (*ConnectionString, error) {
	//u, err := url.Parse(databaseUrl) //replaced
	u, err := newUrlParse(databaseUrl)
